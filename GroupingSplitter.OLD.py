...
parser.add_argument("length", type=int, help="Provide the length of the substring - notepad++ column")
...
try:
    file_in = args.file
    start_in = int(args.start)
    length_in = int(args.length)
    
    start_in_fixed = start_in - 1
    end_in_fixed = start_in_fixed + length_in
except:
...
if not typecheck(start_in_fixed, int):
    print("\n" + "Error: Provided number is not an integer. Terminating")
    sys.exit(9)

if not typecheck(end_in_fixed, int):
    print("\n" + "Error: Provided number is not an integer. Terminating")
    sys.exit(9)    
...
with open(file_in, 'r') as f:
    ...
    for i, lines in enumerate(f):
        ...
        # parse group
        #group = lines[2:6]
        group = lines[start_in_fixed:end_in_fixed]
